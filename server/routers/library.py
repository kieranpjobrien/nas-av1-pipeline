...
def get_library_completion() -> dict:
    ...
    # `audit_summary.buckets` aggregate. The aggregate drifts: when a
    # file gets re-encoded its per-file blob moves to `optimal`, but
    # the summary is only regenerated by the audit tool's full pass.
    # Observed gap on 2026-05-27: summary said 753 to-go, per-file said
    # 186 — KPI button advertised 753 while the drill (which iterates
    # per-file) found 186. Operator hit the mismatch and rightly asked
    # which number was correct.
    #
    # Server-side and client-side now agree because both consume the
    # per-file blob as the source of truth. The aggregate stays in the
    # report (we still read audited_at from it) but is no longer load-
    ...
    for f in files:
        ...
        # `inferred_uncertain` is deliberately not counted in the KPI
        # denominator — the audit treats it as "not enough confidence
